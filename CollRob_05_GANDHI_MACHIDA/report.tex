...
Task 1 tests whether the microscopic locust simulation can be described by the
macroscopic urn model from the lecture.  The full simulation uses $N = 50$
locusts on a continuous one-dimensional ring with $C = 0.5$, speed $= 0.01$,
perception range $r = 0.045$ and spontaneous switching probability $P = 0.15$.

For each of $5000$ independent runs, the swarm is first simulated for $100$ time
steps so that it can settle into an organized collective pattern.  During the
following $20$ time steps we record the one-step change
\[
    \Delta L = L_t - L_{t-1}.
\]
Each observed change is binned by the previous state $L_{t-1}$.  For every
possible value $L \in \{0,\dots,50\}$, the empirical function is estimated as
\[
    \Delta L(L) =
    \frac{\text{sum of observed one-step changes from } L}
         {\text{number of visits to } L}.
\]
States that were not visited are ignored during the fitting step.  The source
code is in \texttt{task\_1/task\_1.py}; the resulting figure is shown in
Figure~\ref{fig:task1}.

\subsection{Subtask a -- The Function \texorpdfstring{$\Delta L(L)$}{Delta L(L)}}

The roots of the measured function,
\[
    \Delta L(L^*) = 0,
\]
are the fixed points of the averaged one-step dynamics.  At such a value $L^*$,
the swarm has no expected tendency to increase or decrease the number of
left-going locusts.

The slope at a root determines its stability.  If the curve crosses the zero
axis with negative slope, $d(\Delta L)/dL < 0$, the fixed point is stable: a
small perturbation creates an average change that pushes the swarm back toward
$L^*$.  If the curve crosses with positive slope, $d(\Delta L)/dL > 0$, the
fixed point is unstable: a small perturbation is amplified and the swarm moves
away.

In the locust swarm this has a direct interpretation.  The symmetric state
$s = 0.5$, or $L = 25$, is the unstable, undecided state.  A perfectly divided
swarm cannot maintain this balance; any slight majority produces positive
feedback and drives the swarm toward one of the two collective directions.  The
stable attractors lie near the edges of the state space, roughly where about
$15\,\%$ or $85\,\%$ of the swarm moves left.  These are cohesive organized
states in which the swarm has committed to a direction.  The unstable fixed
point at $L \approx N/2$ is the same barrier whose crossing is measured in
Task~2.
...
The measured data are collected as $\Delta L$ against $L$, while the model is
written as $\Delta s$ against $s$.  Before fitting, the data are therefore
converted by
\[
    s = \frac{L}{N}, \qquad \Delta s = \frac{\Delta L}{N}.
\]
We then fit Equation~\eqref{eq:ds} with SciPy's nonlinear least-squares
optimizer, using bounds $c \ge 0$ and $0 \le \varphi \le 1$.  Over the $5000$
independent runs, the best-fit parameters are
\[
    c = 0.1666, \qquad \varphi = 1.0000.
\]
The fitted value of $\varphi$ reaches the upper bound, which means the data are
best explained by the strongest positive-feedback profile allowed by the model.

\subsection{Subtask c -- Data, Fit and Probability of Positive Feedback}

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{task_1/plots/urn_model_analysis.png}
    \caption{Left: empirical expected state change $\Delta s(s)$ together with
             the fitted urn model.  Right: the resulting probability of
             positive feedback $P_{FB}(s,\varphi)$ with $\varphi = 1.0$.}
    \label{fig:task1}
\end{figure}

Figure~\ref{fig:task1} shows that the fitted urn model tracks the empirical
data closely.  The left panel captures the expected sign of the one-step change:
near the undecided middle state the dynamics push away from $s = 0.5$, while
near the polarized states the dynamics point back toward the stable attractors.
This confirms that the simple macroscopic equation captures the main emergent
behavior of the microscopic agent-based simulation.

The right panel explains why the middle state is unstable.  With
$\varphi = 1.0$, the positive feedback probability
$P_{FB}(s,\varphi) = \sin(\pi s)$ reaches its maximum value at $s = 0.5$.  When
the swarm is least organized, individual locusts therefore experience the
strongest possible tendency to align with their local neighbors.  The
spontaneous switching probability of $15\,\%$ adds noise, but it does not remove
the strong majority-driven feedback that pushes the swarm toward one of the two
polarized collective states.

The agreement is not expected to be perfect at every state, because the urn
model keeps only the macroscopic variable $s$ and discards the spatial
arrangement of locusts on the ring.  Still, for the purpose of describing the
average one-step dynamics, the fitted urn model is successful: it reproduces the
locations and roles of the unstable undecided state and the two stable organized
states.
...
\midrule
1 & Measurement of $\Delta L(L)$ over ${\approx}5000$ runs      & \checkmark \\
1 & Plot of $\Delta L(L)$; meaning of the zeros $L^*$           & \checkmark \\
1 & Fit of the swarm urn model ($c$ and $\varphi$)              & \checkmark \\
1 & Plot of data, fit and $P_{FB}$                              & \checkmark \\
\midrule
