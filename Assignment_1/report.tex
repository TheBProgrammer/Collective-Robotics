...
We model a computer job queue using a \textbf{Poisson arrival process}.
...
Draws samples from the Poisson distribution using Knuth's inversion algorithm:
starting from a uniform random product $p \leftarrow \prod U_k$, we count how
many factors are needed before $p$ falls below $e^{-\lambda}$.
This requires only the standard \texttt{random} module.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{task_1/task1a_poisson_pmf.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{task_1/task1b_sampling.png}
...
duration of 4 steps per job.  The resulting \textbf{average queue length was
approximately 0.16 jobs}.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.85\textwidth]{task_1/task1d_avg_queue_4steps.png}
...
$\alpha \to 0.25$ from below.  The sharp increase near saturation is characteristic
of an M/D/1 queue (Poisson arrivals, deterministic service, one server).
...
\textbf{2 steps/job} with $\alpha \in [0.005,\,0.5]$ alongside the 4-step case.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{task_1/task1e_comparison.png}
...
explodes shifts from $\approx 0.25$ to $\approx 0.50$.  The shape of the
curve is identical -- a flat region followed by a steep rise near saturation --
but the entire curve is stretched along the $\alpha$ axis by a factor of two.
This confirms that the saturation threshold is governed purely by the ratio
...
applies a \textbf{local majority rule} -- if more than half of its neighbours
